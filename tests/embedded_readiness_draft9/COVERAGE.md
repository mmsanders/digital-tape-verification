...
| WP13-G5 | No indirect call outside exactly four named funnels in `dev.h` | Complete hashed `.c`/`.h` source inventory and complete call-expression classification; exact `dev_read`/`dev_write`/`dev_flush`/`dev_progress` target set | Fifth wrapper, outside-`dev.h` call, wrong progress target/type, missing progress funnel, ambiguity, and incomplete inventories |
...
- missing, duplicate, malformed, or incomplete engine source inventory;
- missing or incomplete call-expression inventory;
...
- an indirect-call whitelist other than the exact four DRAFT-9 named funnels;
- a `dev_progress` target or callback type other than caller-supplied
  `tape_progress_fn`;
