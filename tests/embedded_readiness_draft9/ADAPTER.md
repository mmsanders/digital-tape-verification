...
This package was authored from authenticated DRAFT-9 candidate requirements before
inspecting Product PR #241 or the product engine implementation. After PM issuance,
a later Software round may mechanically bind an exact product candidate to this
package; Software must not alter verifier-owned files.
...
The JSON provenance must include `spec_bundle: "DRAFT-9"` and a `spec_hashes`
object exactly equal to the three hashes recorded in `oracle.py` and the embedded
`spec/VERSION.md`. The package fails closed on any other revision or byte identity.
...
through one of the four named callback funnels `dev_read`, `dev_write`,
`dev_flush`, or `dev_progress`; report those edges explicitly. No other
unknown/indirect edge can be excluded from the bound.
...
- `indirect_calls.source_inventory_complete`;
- `indirect_calls.source_inventory`;
- `indirect_calls.call_expression_inventory_complete`;
- `indirect_calls.call_expression_inventory`;
...
- `dev_read` targeting `tape_dev.read` in `engine/src/dev.h`;
- `dev_write` targeting `tape_dev.write` in `engine/src/dev.h`;
- `dev_flush` targeting `tape_dev.flush` in `engine/src/dev.h`;
- `dev_progress` targeting only the caller-supplied `tape_progress_fn` in
  `engine/src/dev.h`, with `callback_type: "tape_progress_fn"`.

Every syntactic call expression must appear in the call-expression inventory with
path, line, expression and one classification: `direct`, `permitted_indirect`,
`forbidden_indirect`, or `ambiguous`. The four permitted rows in that complete
inventory must equal the separately reported permitted-site set. A missing source,
missing call expression, fifth permitted-looking wrapper, wrong `dev_progress`
target/type, indirect call outside `dev.h`, or ambiguous/unresolved expression is
red.

The `tape_dup` device-identity comparison is not an indirect call and grants
...
- whitelist an additional indirect-call site or omit the required `dev_progress`
  funnel;
