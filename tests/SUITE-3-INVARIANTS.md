# Suite 3 invariant checklist (DRAFT-4 review state)

Status: reconciled to DRAFT-4. Operation adapters remain provisional where `findings/spec-review-draft4.md` identifies incomplete allowed-state sets.
...
7. Re-spool refuses with `TAPE_ERR_CARTRIDGE_FULL` when no pass-1 destination of `len` chunks satisfies the floor and disjointness rules, without changing the mounted cartridge state. Empty-side behavior awaits V4-005.
...
10. Each crash point in either promote phase resolves to one complete, explicitly permitted A/B generation pair and every referenced frame is intact. The phase-2 mixed/low states and exact-tail resume behavior await V4-003/V4-004.
...
16. Entry physical-frame intervals within an index are pairwise disjoint; the scalar capacity inequality in DRAFT-4 is not an acceptable oracle (V4-002).
17. A `version_minor > 0` mount performs no writes and every mutator is refused even when the underlying device has a non-NULL write callback (V4-001).
18. Unknown superblock state values fail closed before repair or mutation (V4-012).
...
- Generated operations must include reset, overwrite, overdub, splice, re-spool, promote, remount, and no-op/boundary operations once the DRAFT-4 state-matrix findings are dispositioned.
