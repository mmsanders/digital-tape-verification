...
def boundary(mount_fixture,rate,seek=None):
    ...
    rendered=2 if rate==-2147483648 else 1
    c += [call("tape_set_rate",rate_q16_16=rate),call("tape_service",block_budget=1024,more_work=False),call("tape_render",requested=2,rendered=rendered),call("tape_tell",frame=1 if mount_fixture=="one" else 0),call("tape_status",at_end=mount_fixture=="one",at_start=mount_fixture!="one"),call("tape_unmount")]
...
def side(playing):
    ...
    c += [call("tape_render",18,requested=1,rendered=0),call("tape_service",block_budget=1024,more_work=False),call("tape_render",requested=1,rendered=1),call("tape_unmount")]
