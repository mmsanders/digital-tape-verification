/* Verifier transcription of the DRAFT-8 public API used by wp06a_probe.c.
 * Standalone syntax/audit aid only. Product integration MUST use the real public
 * header through TAPE_PUBLIC_HEADER rather than this transcription.
 */
#ifndef WP06A_CANDIDATE_API_H
#define WP06A_CANDIDATE_API_H
#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
...
typedef struct {
  const void *data;
  uint32_t data_bytes,valid_frames,start_frame;
  uint8_t uuid[16];
  tape_side side;
} tape_warm_start;
...
typedef struct {
  ...
  uint32_t total_chunks,free_chunks,entry_count,entries_free;
  uint16_t version_minor;
  bool writable,side_b_valid,needs_repair,warm_start_used;
} tape_info;
