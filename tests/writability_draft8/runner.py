#!/usr/bin/env python3
"""Run verifier-owned WP-06a expectations against the verifier product probe."""
...
def main(argv=None):
    ...
    failures=0
    with a.log.open("w") as log,tempfile.TemporaryDirectory(prefix="wp06a-") as td:
        ...
        for case in cases:
            ...
            try:
                ...
                if r.returncode:
                    ...
                    if obs.get("device_write_nonnull") is not True:
                        errors.append("writable-device attestation missing/false")
                    if not out.is_file():
