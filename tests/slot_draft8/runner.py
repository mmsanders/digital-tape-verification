#!/usr/bin/env python3
"""Run verifier-owned WP-36 expectations against a separately linked product adapter."""
...
def main(argv=None):
    ...
    failures=0
    with a.log.open("w") as log,tempfile.TemporaryDirectory(prefix="wp36-slot-") as td:
        ...
        for case in cases:
            ...
            try:
                ...
                if r.returncode:
                    errors.append("adapter process failed; assertion/crash is a WP-36 failure")
                else:
                    obs=json.loads(r.stdout)
                    if obs.get("adapter_kind")!="product":
                        errors.append("non-product adapter refused")
                    if obs.get("debug_assertions") is not True:
                        errors.append("debug assertions not attested enabled")
                    if not out.is_file():
                        errors.append("adapter did not produce output media")
                    else:
                        errors.extend(check(case,Media.decode(out.read_bytes()),obs))
